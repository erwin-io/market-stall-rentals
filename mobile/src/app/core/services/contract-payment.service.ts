...
export class ContractPaymentService implements IServices {
  ...
  update(id: string, data: any): Observable<ApiResponse<ContractPayment>> {
    return this.http.put<any>(environment.apiBaseUrl + this.appconfig.config.apiEndPoints.contractPayment.update + id, data)
    .pipe(
      tap(_ => this.log('contractPayment')),
      catchError(this.handleError('contractPayment', []))
    );
  ...
  updateStatus(id: string, data: any): Observable<ApiResponse<ContractPayment>> {
    return this.http.put<any>(environment.apiBaseUrl + this.appconfig.config.apiEndPoints.contractPayment.updateStatus + id, data)
    .pipe(
      tap(_ => this.log('contractPayment')),
      catchError(this.handleError('contractPayment', []))
    );
